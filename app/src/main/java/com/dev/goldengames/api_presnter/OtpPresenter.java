...
public class OtpPresenter extends BasePresenter<IOtpView> {
    ...
    public void login(final Context context,  Map<String, Object> map ) {
        ...
        AppController.getInstance().getApiInterface().otp(map).enqueue(new Callback<BasicModel>() {
            @Override
   ...
   public void forgot_otp(final Context context,  Map<String, Object> map ) {

        getView().enableLoadingBar(context, true, context.getString(R.string.msg_please_wait));

        AppController.getInstance().getApiInterface().forgot_otp(map).enqueue(new Callback<BasicModel>() {
            @Override
            public void onResponse(@NonNull Call<BasicModel> call, @NonNull Response<BasicModel> response) {
                getView().enableLoadingBar(context, false, "");
                ...
                if (!handleError(response)) {
                    if (response.isSuccessful() && response.code() == 200) {
                        getView().onForgot(response.body());
                    }
                }else{
                    getView().onError(response.message());
                }
