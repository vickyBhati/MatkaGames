...
import com.dev.goldengames.adapter.AdapterHalfPanna;
import com.dev.goldengames.api_presnter.HalfSangamPresenter;
import com.dev.goldengames.databinding.HalfPannaBinding;
...
public class HalfPanna extends BaseActivity implements AdapterHalfPanna.OnItemClickListener, IHalfView {

    HalfPannaBinding binding;
    String openFirst,closeFirst,amountFirst,gameid,bhav;
    ArrayList<HalfSang> myArrList= new ArrayList<HalfSang>();
    AdapterHalfPanna adapter;
    JSONArray jsonArray =new JSONArray();
    HalfSangamPresenter presenter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding  = DataBindingUtil.setContentView(this,R.layout.half_panna);
        ...
        binding.name.setText("Half Sangam");
        binding.mytool.ll.setVisibility(View.VISIBLE);

        userinfo= AppPreference.getUserInfo(HalfPanna.this);
        ...
        binding.etCloseOne.setAdapter(adapter);//setting the adapter data into the AutoCompleteTextView
        ...
        binding.etOpenTwo.setThreshold(3);//will start working from first character
        binding.etOpenTwo.setAdapter(adapter2);//setting the adapter data into the AutoCompleteTextView
        binding.etOpenTwo.setTextColor(Color.BLACK);
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (start==2){
                    if(Arrays.asList(digit).contains(s.toString())){
                        check="1";
                        Toast.makeText(HalfPanna.this,"valid",1000).show();
                    }
                    else {
                        check="2";
                        Toast.makeText(HalfPanna.this," Not valid",1000).show();
        ...
        binding.etOpenTwo.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        ...
        presenter = new HalfSangamPresenter();
        ...
        switch (view.getId()){
            case R.id.btnOne:
                if (TextUtils.isEmpty(binding.etOpenOne.getText().toString())){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  Open value ");

                }
                else if (TextUtils.isEmpty(binding.etCloseOne.getText().toString())){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  Close value ");
                }
                else if (binding.etCloseOne.getText().toString().length()<3){
                    Utility.showErrorToast(HalfPanna.this,"Please enter valid Close value ");
                }
                else if (!check.equals("1")){
                    Utility.showErrorToast(HalfPanna.this,"Not a valid close value");
                }


                else if (TextUtils.isEmpty(binding.etAmoutOne.getText().toString())){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  Amount ");
                }
                else if (Integer.parseInt(binding.etAmoutOne.getText().toString()) < 10 ){
                    Utility.showErrorToast(HalfPanna.this,"Amount Must be greater 10");
                }
                    ...
                    myArrList.add(new HalfSang(binding.etOpenOne.getText().toString()+"-"+binding.etCloseOne.getText().toString(),amountFirst,"Open-Close"));
                    addView();
                }
                break;
            case R.id.btnTwo:
                if (TextUtils.isEmpty(binding.etCloseTwo.getText().toString())){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  Close value ");
                }
                else if (TextUtils.isEmpty(binding.etOpenTwo.getText().toString())){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  Open value ");
                }
                else if (binding.etOpenTwo.getText().toString().length()<3){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  valid Open value ");
                }
                else if (!check2.equals("1")){
                    Utility.showErrorToast(HalfPanna.this,"Not a valid open value");
                }


                else if (TextUtils.isEmpty(binding.etAmountTwo.getText().toString())){
                    Utility.showErrorToast(HalfPanna.this,"Please enter  Amount ");
                }
                else if (Integer.parseInt(binding.etAmountTwo.getText().toString()) < 10 ){
                    Utility.showErrorToast(HalfPanna.this,"Amount Must be greater 10");
                }
                else {
                    String number = binding.etOpenTwo.getText().toString()+"-"+binding.etCloseTwo.getText().toString();
                    String amountFirst = binding.etAmountTwo.getText().toString();
                    myArrList.add(new HalfSang(number,amountFirst,"Close-Open"));
                    addView();
                }
                break;
            case R.id.btnEnglish:
            if(myArrList.size()==0){
                Utility.showErrorToast(HalfPanna.this,"Please add bet");
            }
            else {
                ...
                for (int i = 0; i < myArrList.size(); i++) {
                    try {
                        ...
                        jsonObject.put("bet_type", myArrList.get(i).getBet_type());
...
//                    }
                ...
                if (isopen.equals("0")){
                    Utility.showErrorToast(HalfPanna.this,"Market either close or not open ");
                }
                else {
                    if (Utility.hasConnection(this)) {
                        ...
                        map.put("game_type",""+"half-sangam");
                        map.put("bet_no",""+jsonArray);
                        map.put("bhav",""+bhav);
                        Log.e("tttt",""+map);
                        presenter.halfsangam(this, map);
                    }
                    else {
    ...
    private void addView() {
        ...
        adapter = new AdapterHalfPanna(HalfPanna.this,this,myArrList);
        binding.recyclerView.setLayoutManager(new LinearLayoutManager(HalfPanna.this));
    ...
    @Override
    public void RemoveList(View view, int index, HalfSang item) {
    ...
    public void onUpdate(BasicModel body) {
        if (body.getStatus()==1){
            ...
            Utility.showSuccesToast(HalfPanna.this,body.getMessage());
//            MainActivity.wallet="1";
            AppPreference.wallet(getApplicationContext(), String.valueOf(body.getWallet()));

//            Intent intent = new Intent(HalfPanna.this,MainActivity.class);
...
//            finishAffinity();
            ...
            Utility.showErrorToast(HalfPanna.this,body.getMessage());
