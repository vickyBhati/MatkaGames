...
import com.dev.goldengames.adapter.WingStatment;
...
//                if (list.get(i).getTransaction_type().equalsIgnoreCase("Winning")){
//                    two.addAll(list);
//                }
//            }
            for (int i = 0; i < list.size(); i++) {
                TrasnscationModel.DataBean b = list.get(i);
                if (b.getTransaction_type().equalsIgnoreCase("Winning")){
                    two.add(b);
                }
            }
            ...
            WingStatment adapter = new WingStatment(getActivity(),two);
